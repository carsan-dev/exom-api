...
import {
  AdminClientProgressQueryDto,
  ReplyToTrainingNoteDto,
} from './dto/admin-client-progress-query.dto';
import {
...
export class UsersController {
  ...
  @ApiResponse({
    status: 200,
    description: 'Listado global de usuarios obtenido correctamente',
  })
  ...
  @ApiOperation({
    summary:
      'Reenviar email de invitación para que el usuario fije su contraseña',
  })
  @ApiResponse({
    status: 201,
  ...
  @ApiResponse({
    status: 403,
    description: 'Solo SUPER_ADMIN puede consultar asignaciones',
  })
  ...
  getClientAssignments(
    ...
    return this.usersService.getClientAssignments(
      admin.id,
      admin.role,
      clientId,
    );
  ...
  @Roles(Role.SUPER_ADMIN)
  @ApiOperation({
    summary: 'Actualizar de forma atomica los admins activos de un cliente',
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description: 'Solo SUPER_ADMIN puede actualizar asignaciones',
  })
  ...
  updateClientAssignments(
    ...
    return this.usersService.updateClientAssignments(
      admin.id,
      admin.role,
      clientId,
      dto,
  ...
  @ApiResponse({
    status: 403,
    description: 'El cliente no está asignado al admin actual',
  })
  @ApiResponse({ status: 404, description: 'Cliente no encontrado' })
  getClientProfile(
    @CurrentUser() admin: AuthenticatedUser,
  ...
  @ApiResponse({
    status: 200,
    description: 'Progreso del día obtenido correctamente',
  })
  ...
  getClientDayProgress(
    ...
    return this.usersService.getClientDayProgress(
      admin.id,
      admin.role,
      clientId,
      query.date,
  ...
  @Put('clients/:id/progress/reply')
  @Roles(Role.SUPER_ADMIN, Role.ADMIN)
  @ApiOperation({
    summary: 'Responder a la nota diaria de entrenamiento de un cliente',
  })
  @ApiResponse({ status: 200, description: 'Respuesta guardada correctamente' })
  @ApiResponse({
    status: 400,
    description: 'El progreso no contiene una nota del cliente',
  })
  @ApiResponse({ status: 403, description: 'Sin acceso a este cliente' })
  @ApiResponse({ status: 404, description: 'Progreso del día no encontrado' })
  replyToTrainingNote(
    @CurrentUser() admin: AuthenticatedUser,
    @Param('id') clientId: string,
    @Body() dto: ReplyToTrainingNoteDto,
  ) {
    return this.usersService.replyToTrainingNote(
      admin.id,
      admin.role,
      clientId,
      dto.date,
  ...
  @ApiResponse({
    status: 200,
    description: 'Calendario mensual obtenido correctamente',
  })
  getClientCalendarMonth(
    ...
    return this.usersService.getClientCalendarMonth(
      admin.id,
      admin.role,
      clientId,
      query.year,
  ...
  @ApiResponse({
    status: 200,
    description: 'Resumen semanal obtenido correctamente',
  })
  getClientWeekSummary(
    ...
    return this.usersService.getClientWeekSummary(
      admin.id,
      admin.role,
      clientId,
      query.week_start,
  ...
  @ApiResponse({
    status: 200,
    description: 'Métricas del cliente obtenidas correctamente',
  })
  getClientMetrics(
    @CurrentUser() admin: AuthenticatedUser,
    @Param('id') clientId: string,
    ...
    return this.usersService.getClientMetrics(
      admin.id,
      admin.role,
      clientId,
      pagination,
  ...
  @ApiResponse({
    status: 409,
    description: 'Ya existe una métrica para esa fecha',
  })
  createClientMetric(
    @CurrentUser() admin: AuthenticatedUser,
    @Param('id') clientId: string,
    ...
    return this.usersService.createClientMetric(
      admin.id,
      admin.role,
      clientId,
      dto,
  ...
  getClientWeightHistory(
    ...
    return this.usersService.getClientWeightHistory(
      admin.id,
      admin.role,
      clientId,
    );
  ...
  @ApiResponse({
    status: 200,
    description: 'Historial de medida obtenido correctamente',
  })
  getClientBodyHistory(
    ...
    return this.usersService.getClientBodyHistory(
      admin.id,
      admin.role,
      clientId,
      query.field,
